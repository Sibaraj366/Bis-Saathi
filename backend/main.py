    ...
    for pattern in patterns:
        ...
        for match in re.finditer(
            ...
            if len(groups) == 4:
                ...
                part_number = groups[1] or groups[2]
...
def prioritize_legacy_results(
    legacy_results,
    semantic_results,
    top_k: int = 6,
):
    """
    Preserve trusted V1.3 retrieval order while allowing
    semantic-only RAG results to be appended.
    """

    ordered = []

    for legacy_item in legacy_results or []:
        legacy_is = normalize_is_number(
            legacy_item.get("is_number", "")
        )

        matching = None

        for semantic_item in semantic_results or []:
            semantic_is = normalize_is_number(
                semantic_item.get("is_number", "")
            )

            if (
                legacy_is
                and semantic_is
                and legacy_is == semantic_is
            ):
                matching = semantic_item
                break
...
def build_chat_context(
    ...
    # --------------------------------------------------------
    # V1.3 product + voltage-aware retrieval
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Preserve V1.3 authoritative standards
    #
    # These results already contain:
    # - exact IS matching
    ...
    # --------------------------------------------------------
    # Exact IS-number lookup is authoritative.
    #
    # If the user explicitly names an IS number, do not merge
    # product or semantic results into the authoritative list.
    # This prevents unrelated standards from appearing in
    # matchedStandards for questions such as:
    # "Tell me about IS 269:2015".
    # --------------------------------------------------------

    if exact:
        legacy_standards = exact[:3]

        # For exact lookups, keep generation context restricted
        # to the exact standard as well.
        rag_results = legacy_standards

    else:
        # ----------------------------------------------------
        # Normal product/general question. Preserve V1.3
        # product-family and voltage-aware retrieval, then
        # enrich it with RAG V2.1 semantic retrieval.
        # ----------------------------------------------------

        legacy_standards = combine_standard_results(
            exact,
            related,
            top_k=6,
        )
        ...
        try:
            semantic_results = rag_v21_search(
                question,
                legacy_standards,
                top_k=6,
            )

            rag_results = prioritize_legacy_results(
                legacy_standards,
                semantic_results,
                top_k=6,
            )
    ...
    # --------------------------------------------------------
    # Keep V1.3 standard objects for API response.
    #
    # matchedStandards remains based on the trusted
    # exact/product/voltage-aware retrieval.
    # --------------------------------------------------------

    standards = legacy_standards

    # For exact standard lookups, do not expose product-search
    # results as additional matches.
    if exact:
        extra = []
    else:
        extra = [
...
def rag_search(
    ...
    # Preserve V1.3 retrieval as the authoritative layer.
    exact = retrieve_standard_by_is_number(
        question,
        top_k=3,
    )

    # If the user asks for an exact IS number,
    # return only the matching standard.
    if exact:
        return {
            "success": True,
            "query": question,
    ...
    try:
        ...
        results = prioritize_legacy_results(
            legacy_standards,
            semantic_results,
            top_k=6,
        )
